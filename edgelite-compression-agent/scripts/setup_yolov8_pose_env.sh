...
usage() {
  cat <<'EOF'
Usage:
  bash edgelite-compression-agent/scripts/setup_yolov8_pose_env.sh [options]

Options:
  --env-name NAME        Conda environment name. Default: yolov8-pose
  --tensorrt-dir PATH    TensorRT unpacked directory, e.g. TensorRT-8.6.1.6
  --yes                 Non-interactive mode

Environment variables:
  ENV_NAME              Same as --env-name
  TRT_DIR               Same as --tensorrt-dir
  YES=1                 Same as --yes
...
if [[ -n "${TRT_DIR}" && -d "${TRT_DIR}" ]]; then
  ...
  cat > "${ACTIVATE_DIR}/edgelite_tensorrt.sh" <<EOF
...
EOF
  # shellcheck source=/dev/null
  source "${ACTIVATE_DIR}/edgelite_tensorrt.sh"
else
  echo "[warn] TensorRT directory was not provided/found."
  echo "[warn] Python dependencies are installed, but TensorRT engine build requires either:"
  echo "       1) --tensorrt-dir /path/to/TensorRT-8.6.1.6, or"
  echo "       2) system trtexec in PATH, or"
  echo "       3) a compatible TensorRT Python installation."
fi

...
class FeatureLoss(nn.Module):
    ...
    def forward(self, y_s, y_t):
        ...
        tea_feats = []
        stu_feats = []
        # for id, y_s_tensor in enumerate(y_s):
        #     print(f"y_s id{id} shape: {y_s_tensor.shape}")
        # for id, y_t_tensor in enumerate(y_t):
        #     print(f"y_t id{id} shape: {y_t_tensor.shape}")
        for idx, (s, t) in enumerate(zip(y_s, y_t)):
            # change ---
...
class Distillation_loss:
    def __init__(self, modeln, modelL, distiller="CWDLoss"):  # model must be de-paralleled
        ...
        # channels_s = [128, 256, 128, 64, 128, 256] # 剪枝org * 0.5
        channels_s = [123, 250, 123, 58, 123, 250] # 剪枝org * 0.7
        # channels_s = [256, 512, 256, 128, 256, 512][-le:]
        channels_t = [256, 512, 256, 128, 256, 512][-le:]
